...
The match key is `tcp.stream` plus `couchbase.opaque`. Opaque is the client’s semi-transaction number: the request and its reply carry the same value, and the same opaque on another stream is a different call. A Statistics request is answered by many packets on that same opaque, and those packets count as one call. Common display filters for a document, one opaque, an opcode, a status, a large body, and TCP loss are in [CB_WIRESHARK.md](CB_WIRESHARK.md). The in-flight window is the longest matched round trip: a response at the open of the file, and a request at the close, are the capture cutting through calls already on the wire. Unmatched rows farther inside the file are counted with the TCP gaps when a pcap is available.

### Where the command names come from

This project does not ship a Couchbase decoder. Opcode names, DCP flags, and the snapshot ack bit are fields from Wireshark’s dissector, `epan/dissectors/packet-couchbase.c`. That file has lived in Wireshark since 2014. Couchbase engineers send changes into Wireshark rather than keeping a fork. The GitHub tree is a read-only mirror. The file to read is the [GitLab upstream](https://gitlab.com/wireshark/wireshark/-/blob/master/epan/dissectors/packet-couchbase.c). It covers the memcached binary protocol on port 11210, including DCP. The protocol those fields implement is in Couchbase kv_engine: [BinaryProtocol.md](https://github.com/couchbase/kv_engine/blob/master/docs/BinaryProtocol.md) and the [DCP protocol](https://github.com/couchbase/kv_engine/blob/master/docs/dcp/documentation/protocol.md).

### Why DCP is not another client call

A normal KV call is one request and one reply. They share an opaque, and that number belongs to that one call on that TCP stream. DCP is full duplex, and the connection stays open while both sides send. The consumer’s stream request picks an opaque, and every later snapshot, mutation, deletion, and expiration on that stream copies it. Those packets use request magic `0x80`. A mutation does not get a command reply. Flow control is a later buffer acknowledgement, not a reply per document. A second packet with the same opaque and the same key is the next change on the stream, with the sequence number and the revision moved forward. It is not a retry of the first packet, and it is not an application call. The filters and the reply rules are in [CB_WIRESHARK.md](CB_WIRESHARK.md).

### Calls by opcode and response time

![Stacked opcodes with median and p99](images/op_type_vs_speed.png)

Each color is one Couchbase command. Bar height is how many of that command ran in the second. The lines are the application median and the cluster median. A blended line would sit on whichever side sent more calls and hide the other.
...
Every run writes `charts.json` and `index.html` next to the note. The page is grouped. **Timings** opens with latency tiles for the application and for the cluster, each with its own median and p99, plus the slowest call and how many matched calls were at least 100 ms or 250 ms. The histogram, round-trip lines, tail lines, and latency candles use those two speeds. **Operations** is opcode mix and p99. **Packets** is the port-11210 bars, lost responses, and the missing-call tabs. **Clients** starts with **Cluster**: node-to-node and DCP or replication-meta commands, on their own charts, plus a time chart of application requests against cluster requests, missing replies, and retransmissions. **Client fleet** under that is application ports only, so a replication stream does not sit in the client trouble list. **Look up a client** opens the application addresses when you click it, missing replies first, and the box narrows that list. A copy icon on a row, a stream, an opcode, a slow call, and a packet bar pastes a Wireshark filter. The report ends with those filters too. The tiles show an SDK median and a cluster median. Missing calls, the slowest calls, and the opcode list are marked `sdk` or `cluster`. **Documents** is body length and the ten-id tables. **Marks** is the points of interest. **Report / Questions** opens the formatted note and lists the next questions. **Glossary** follows. On a wide window, related charts sit side by side. A side panel switches the time bucket (0.5, 1, 5, or 10 seconds) and switches every numeric axis between linear and log. The time charts share one crosshair. Double-click drops a stake on that second. The stake stays when a legend series is hidden. Each chart has an expand control that opens it on a black transparent overlay and keeps the current zoom and stakes.

The ten slowest matched calls are listed with the response time, the second the request was sent, and the body size. **Set Stake** marks that second on the other charts, which is how a single slow call is placed back into the trend. Missing calls are four tabs, application first, then cluster. Each tab shows up to ten rows spaced across that side, and the tab label is the full count. Every unanswered request is in `orphans.tsv`. **Capture duplicate** on the packet chart is the same TCP segment recorded twice within a millisecond. That bar starts hidden. The glossary explains it. **Next questions / steps** is written from the same counts as the note.

The same page also has the latency histogram, round-trip lines, a chart of both medians and both p99s with dashed TCP-loss and retry lines, a min-to-max candle for each side, and p99 by opcode. Client addresses are in **Clients**.
